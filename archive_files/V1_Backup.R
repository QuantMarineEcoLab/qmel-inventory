              ...
              tabPanel(icon = icon("shopping-cart"), "Checkout/Return (WIP)", 
                       ...
                       sidebarLayout(
                         ...
                         mainPanel(
                           ...
                           fluidRow(column(12, actionButton("submitCheckout", "Submit checkout",
                                                            style="color: #fff; background-color: #337ab7; border-color: #2e6da4")))
                         )
                       )
              ),
...
server <- function(input, output, session) {
  ...
  # Reactive table holding all checked-out items
  checkout_list <- reactiveValues(df = checkout)
  
  # Temporary cart to store items before checkout is submitted
  cart_items <- reactiveValues(df = data.frame())
  
  # Add selected item to the cart
  # reactive table allows the program to take a snapshot of the user input without modifying the table yet

  observeEvent(input$addToCart, {
    
    req(input$checkout_name,
        input$checkout_item,
        input$checkout_quant,
        input$checkout_return)
    ...
    new_row <- data.frame(
      ...
      start_date = input$checkout_return[1],
      end_date   = input$checkout_return[2],
      stringsAsFactors = FALSE
    )
    
    cart_items$df <- bind_rows(cart_items$df, new_row)
  })
  
  # Display cart contents "Items in your cart" table
  output$cart <- renderDT({
    datatable(cart_items$df, rownames = FALSE)
  })
  
  # Submit checkout: move cart items into checkout list and save to checkout.csv
  observeEvent(input$submitCheckout, {
    
    req(nrow(cart_items$df) > 0)
    
    message("you've checked out!")
    
    checkout_list$df <- bind_rows(checkout_list$df, cart_items$df)
    
    fwrite(checkout_list$df, "checkout.csv", append = TRUE)
    
    cart_items$df <- data.frame()
  })
  
  
  # Display items currently checked out by selected user
  output$myItems <- renderDT({
    req(input$checkout_name)
    
    datatable(
      checkout_list$df %>% filter(name == input$checkout_name),
      rownames = FALSE
    )
  })
  
  # Return selected items and update checkout file
  observeEvent(input$returnItems, {
    
    req(input$myItems_rows_selected)
    
    checkout_list$df <- checkout_list$df %>%
      slice(-input$myItems_rows_selected)
    
    fwrite(checkout_list$df, "checkout.csv")
  })
